...
# -----------------------------------------------------------------------------
# vcov = HC4 (cluster-robust)
# -----------------------------------------------------------------------------

cluster_robust_se_hc4 <- function(model, cluster_array){
  ...
  # hat mean
  hat_mean <- mean(hatvalues)
  hat_over_mean <- hatvalues/hat_mean
  
  # delta
  model$model$delta <- pmin(4,hat_over_mean)
  
  # ühat
  uhat <- resid(model) / sqrt((1 - hatvalues)^model$model$delta)
...
coeftest(plm, vcov=cluster_robust_se_hc4(plm, cluster))
coeftest(plm, vcov=vcovHC(plm, type="HC4", cluster="group"))
